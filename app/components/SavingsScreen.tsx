...
import { View, Text, ScrollView, TouchableOpacity, TextInput, Alert, Modal, Pressable, Image } from 'react-native';
import { Plus, ChevronRight, Trash2, Calendar, DollarSign, X } from 'lucide-react-native';
import { SafeAreaView } from 'react-native-safe-area-context';

// Define the icon types
type GoalIcon = 'averagePrice' | 'bookmark' | 'briefcase' | 'car' | 'dollarBag' | 'favorite' | 'happy' | 'home' | 'idea' | 'keySecurity' | 'key' | 'lock' | 'wallet' | 'traveler' | 'airport' | 'gift' | 'goal';
...
const colors = [
  "#FFCCCB", "#ADD8E6", "#FFD580", "#90EE90", "#E6E6FA",
  "#FFB6C1", "#FFFF99", "#B0E0E6", "#FFA07A", "#98FB98"
];

export default function SavingsScreen() {
  const [goals, setGoals] = useState<Goal[]>([
    { id: '1', name: 'Emergency Fund', target: 5000, saved: 3200, deadline: '2023-12-31', icon: 'dollarBag', iconColor: "#90EE90" },
    { id: '2', name: 'New Laptop', target: 1200, saved: 450, deadline: '2023-08-15', icon: 'goal', iconColor: "#ADD8E6" },
    { id: '3', name: 'Vacation', target: 3000, saved: 1200, deadline: '2024-03-01', icon: 'traveler', iconColor: "#B0E0E6" },
  ]);
      ...
      <ScrollView className="flex-1 p-4">
        <View>
          <View className="flex-row justify-between items-center mb-6">
            <Text className="font-bold text-xl text-gray-900">Savings Goals</Text>
            <TouchableOpacity 
          ...
          {goals.length === 0 ? (
            <View className="py-8 items-center">
              ...
              <Text className="text-gray-500 text-lg text-center">No savings goals yet</Text>
              <Text className="text-gray-400 text-center mt-2">Tap the + button to add your first goal</Text>
            </View>
          ) : (
                  ...
                  >
                    <View className="flex-row justify-between items-start mb-3">
                      <View className="flex-row items-center">
                        <View className="p-2 rounded-full mr-3" style={{ backgroundColor: goal.iconColor }}>
                          <Image source={goalIcons[goal.icon]} className="w-6 h-6" />
                        </View>
                        <View>
                          <Text className="font-semibold text-gray-900 text-lg">{goal.name}</Text>
                    ...
                    <View className="flex-row justify-end mt-2">
                      <Text className="font-medium text-gray-900">
                        ${goal.saved.toFixed(2)}<Text className="text-gray-400"> / ${goal.target.toFixed(2)}</Text>
                      </Text>
                    </View>
                  </Pressable>
                );
            ...
            <View className="space-y-5">
              ...
              <View className="flex-row space-x-4 gap-2 items-center">
                <View className="flex-1">
                  ...
                  >
                    <View className="rounded-full mr-3" style={{ backgroundColor: formData.iconColor }}>
                      <Image source={goalIcons[formData.icon]} className="w-10 h-10" />
                    </View>
                ...
                <View className="flex-1">
                  ...
                  >
                    <View className="w-10 h-10 rounded-full mr-3" style={{ backgroundColor: formData.iconColor }} />
              ...
              <View>
                <Text className="text-gray-700 mb-2 font-medium">Goal Name</Text>
                <TextInput
              ...
              <View>
                ...
                <View className="flex-row items-center border border-gray-200 rounded-xl bg-gray-50">
                  ...
                  <TextInput
                    ...
                    onChangeText={(text) => handleInputChange('target', text.replace(/[^0-9.]/g, ''))}
                  />
                </View>
              </View>

              <View>
                <Text className="text-gray-700 mb-2 font-medium">Amount Saved</Text>
                <View className="flex-row items-center border border-gray-200 rounded-xl bg-gray-50">
                  <View className="px-4">
                    <DollarSign size={18} color="#6b7280" />
                  </View>
                  <TextInput
                    ...
                    onChangeText={(text) => handleInputChange('saved', text.replace(/[^0-9.]/g, ''))}
                  />
                </View>
              </View>
              ...
              {Object.keys(goalIcons).map((icon) => (
                ...
                >
                  <View className="p-3 rounded-full mb-2" style={{ backgroundColor: formData.iconColor }}>
                    <Image source={goalIcons[icon as GoalIcon]} className="w-8 h-8" />
                  </View>
