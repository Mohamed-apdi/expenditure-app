    ...
    try {
      ...
      if (activeSubscriptions.length === 0) {
        Toast.show({
          ...
          text2: "Add an active subscription to test notifications",
        });
        return;
      }
      ...
      // Get the first active subscription for testing
      const testSubscription = activeSubscriptions[0];
      const daysUntilPayment = Math.ceil(
        (new Date(testSubscription.next_payment_date).getTime() - new Date().getTime()) /
        (1000 * 60 * 60 * 24)
      );
      ...
      await createNotification({
        user_id: userId,
        title: `Test: ${testSubscription.name} Payment Reminder`,
        message: `Your ${testSubscription.name} subscription of $${testSubscription.amount.toFixed(2)} is due in ${daysUntilPayment} days`,
        type: "subscription",
        priority: "medium",
        metadata: {
          subscription_id: testSubscription.id,
          amount: testSubscription.amount,
          next_payment_date: testSubscription.next_payment_date,
        },
        action_url: "/components/SubscriptionsScreen",
      });

      // Send a local push notification using expo-notifications
      await Notifications.scheduleNotificationAsync({
        content: {
          title: `${testSubscription.name} Payment Reminder`,
          body: `Your subscription of $${testSubscription.amount.toFixed(2)} is due in ${daysUntilPayment} days`,
          data: { subscriptionId: testSubscription.id },
        },
      ...
      Toast.show({
        ...
        text2: `Sent notification for ${testSubscription.name}`,
      });
    } catch (error) {
        ...
        <View style={{ paddingHorizontal: 16, paddingVertical: 16 }}>
          ...
          <View className="flex-row justify-between items-center mb-6">
            <View>
              ...
              <Text style={{ color: theme.textSecondary, fontSize: 14, marginTop: 4 }}>
                {subscriptions.filter(s => s.is_active).length} active • ${totalMonthlyCost.toFixed(2)}/month
              </Text>
            </View>
          ...
          >
            <View className="flex-row items-center mb-2">
              ...
              <Text style={{ color: theme.primaryText, fontSize: 14, fontWeight: "500", opacity: 0.9 }}>
                {t.totalMonthlyCost || "Monthly Cost"}
              </Text>
            </View>
            <Text style={{ color: theme.primaryText, fontSize: 32, fontWeight: "bold", marginBottom: 4 }}>
                        ...
                        <View className="flex-row justify-between items-start mb-3">
                          <View className="flex-row items-center flex-1">
                            ...
                            >
                              <Image
                                source={
                                  serviceIcons[subscription.icon as keyof typeof serviceIcons] ||
                                  serviceIcons.other
                                }
                            ...
                            <View className="flex-1">
                              ...
                              <View className="flex-row items-center mt-1" style={{ gap: 6 }}>
                                ...
                                {subscription.category && (
                                  ...
                                  >
                                    <Text style={{ color: subscription.icon_color, fontSize: 11, fontWeight: "600" }}>
                        ...
                        <View className="flex-row justify-between items-center mb-2">
                          ...
                          <View style={{ alignItems: "flex-end" }}>
                            ...
                            <Text style={{ color: theme.textMuted, fontSize: 11, marginTop: 2 }}>
                              {daysUntilPayment > 0 ? `in ${daysUntilPayment} days` : daysUntilPayment === 0 ? 'Today' : 'Overdue'}
                            </Text>
                          </View>
                        </View>
          ...
          >
            <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <View className="flex-row gap-3">
                  ...
                  >
                    ...
                    <View className="flex-1">
                      ...
                      <Text style={{ color: theme.text, fontSize: 13, fontWeight: "500", textTransform: "capitalize" }}>
                        {formData.icon}
                      </Text>
                    </View>
                  </TouchableOpacity>
                  ...
                  {billingCycles.map((cycle) => (
                    <TouchableOpacity
                      ...
                      style={{
                        ...
                        borderRadius: 12,
                        borderWidth: 1,
                        borderColor: formData.billing_cycle === cycle.key ? theme.primary : theme.border,
                        backgroundColor: formData.billing_cycle === cycle.key ? `${theme.primary}20` : theme.background,
                      }}
                      onPress={() =>
                        setFormData({
                          ...formData,
                          billing_cycle: cycle.key as "weekly" | "monthly" | "yearly",
                        })
                      }
                    >
                      <Text
                        style={{
                          color: formData.billing_cycle === cycle.key ? theme.primary : theme.textSecondary,
                          fontWeight: formData.billing_cycle === cycle.key ? "600" : "400",
                          textAlign: "center",
                          fontSize: 14,
                        }}
                      >
              ...
              >
                ...
                >
                  {isEditMode ? t.updateSubscription || "Update" : t.addSubscription || "Add Subscription"}
                </Text>
              </TouchableOpacity>
            </ScrollView>
          </View>
                ...
                return (
                  <TouchableOpacity
                    ...
                    style={{
                      ...
                      padding: 12,
                      alignItems: "center",
                      backgroundColor: isSelected ? `${formData.icon_color}15` : theme.background,
                      borderRadius: 12,
                      borderWidth: 2,
                      borderColor: isSelected ? formData.icon_color : theme.border,
                    }}
