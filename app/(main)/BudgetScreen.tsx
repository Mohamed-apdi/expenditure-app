...
import { View, Text, ScrollView, TouchableOpacity, Modal, TextInput, Alert, PanResponder } from 'react-native';
...
export default function BudgetScreen() {
  ...
  const [budgets, setBudgets] = useState([
    ...
    { id: '2', category: 'Transport', allocated: 200, spent: 150, color: '#3b82f6' },
    { id: '3', category: 'Entertainment', allocated: 100, spent: 85, color: '#8b5cf6' },
    { id: '4', category: 'Utilities', allocated: 300, spent: 280, color: '#10b981' },
  ]);
  ...
  const handleSave = () => {
    ...
    if (currentBudget) {
      ...
      // Add new budget
      const colors = ['#f59e0b', '#3b82f6', '#8b5cf6', '#10b981', '#ec4899', '#14b8a6'];
      const newBudget = {
  ...
  return (
    ...
    <View className="flex-1" {...panResponder.panHandlers}>
      ...
      <View className="flex-row border-b border-gray-200 bg-white">
        ...
        >
          <Text className={`font-medium ${activeTab === 'Budget' ? 'text-blue-500' : 'text-gray-500'}`}>Budget</Text>
        </TouchableOpacity>
        <TouchableOpacity 
          className={`flex-1 py-4 items-center ${activeTab === 'Subscriptions' ? 'border-b-2 border-blue-500' : ''}`}
          onPress={() => setActiveTab('Subscriptions')}
        >
          <Text className={`font-medium ${activeTab === 'Subscriptions' ? 'text-blue-500' : 'text-gray-500'}`}>Subscriptions</Text>
        </TouchableOpacity>
        <TouchableOpacity 
          className={`flex-1 py-4 items-center ${activeTab === 'Goals' ? 'border-b-2 border-blue-500' : ''}`}
          onPress={() => setActiveTab('Goals')}
        >
          <Text className={`font-medium ${activeTab === 'Goals' ? 'text-blue-500' : 'text-gray-500'}`}>Goals</Text>
        </TouchableOpacity>
      </View>
              ...
              {budgets.length === 0 ? (
                ...
                <View className="flex-row flex-wrap justify-between">
                  {budgets.map((budget) => {
                    ...
                    return (
                      <View key={budget.id} className="w-[48%] mb-4 p-3 bg-white rounded-xl border border-gray-100">
                        <View className="flex-row justify-between items-center mb-2">
                          <Text className="font-medium">{budget.category}</Text>
                        ...
                        <View className="mt-2">
                          <View className="flex-row justify-between">
                            ...
                            <Text className="text-xs font-medium">${budget.spent.toFixed(2)}</Text>
                          </View>
                          <View className="flex-row justify-between">
                            <Text className="text-gray-500 text-xs">Budget</Text>
                            <Text className="text-xs font-medium">${budget.allocated.toFixed(2)}</Text>
                          </View>
                          <View className="flex-row justify-between">
