...
const AddAccount = ({
  ...
  const handleAddAccount = () => {
    if (!newAccount.group || !newAccount.name) {
    ...
    setNewAccount({ group: "", name: "", amount: 0, description: "" });
  };
        ...
        <ScrollView className="flex-1 px-6 pt-6">
          ...
          <View className="mb-5">
            ...
            >
              <Text
                className={`${
                  newAccount.group ? "text-white" : "text-slate-500"
                }`}
              >
                {newAccount.group || "Select group"}
              </Text>
                ...
                renderItem={({ item }) => (
                  <TouchableOpacity
                    className={`px-4 py-3 flex-row justify-between items-center ${
                      newAccount.group === item.name ? "bg-slate-700" : ""
                    }`}
                    onPress={() => {
                      setNewAccount({ ...newAccount, group: item.name });
                      setShowGroupModal(false);
                    }}
                  >
                    <Text className="text-white">{item.name}</Text>
                    {newAccount.group === item.name && (
