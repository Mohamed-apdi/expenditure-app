...
import { fetchAccounts, updateAccount, createAccount, type Account } from './accounts';
  ...
  useEffect(() => {
    const autoLoadAccounts = async () => {
      try {
        ...
        if (user) {
          console.log("AccountContext - Auto-loading accounts for user:", user.id);
          await loadAccounts();
        }
      } catch (error) {
    ...
    try {
      ...
      if (fetchedAccounts && fetchedAccounts.length > 0) {
        ...
        if (!selectedAccount && !isUpdatingDefault) {
          ...
          console.log("Setting initial selected account to:", accountToSelect.name);
    ...
    try {
      ...
      if (fetchedAccounts && fetchedAccounts.length > 0) {
        ...
        if (!selectedAccount && !isUpdatingDefault) {
          const defaultAccount = fetchedAccounts.find(acc => acc.is_default === true);
          const accountToSelect = defaultAccount || fetchedAccounts[0];
          console.log("Setting refreshed selected account to:", accountToSelect.name);
          setSelectedAccountState(accountToSelect);
        }
      } else {
