...
export const fetchSubscriptionsWithAccounts = async (userId: string): Promise<any[]> => {
  const { data, error } = await supabase
    .from('subscriptions')
    .select(`
      *,
...
export const addSubscription = async (subscription: Omit<Subscription, 'id' | 'created_at' | 'updated_at'>): Promise<Subscription> => {
  const { data, error } = await supabase
    .from('subscriptions')
    .insert(subscription)
    .select()
...
export const getSubscriptionsByAccount = async (userId: string, accountId: string): Promise<Subscription[]> => {
  const { data, error } = await supabase
    .from('subscriptions')
    .select('*')
    .eq('user_id', userId)
...
export const getSubscriptionsByCategory = async (userId: string, category: string): Promise<Subscription[]> => {
  const { data, error } = await supabase
    .from('subscriptions')
    .select('*')
    .eq('user_id', userId)
...
export const getSubscriptionsByBillingCycle = async (userId: string, billingCycle: 'weekly' | 'monthly' | 'yearly'): Promise<Subscription[]> => {
  const { data, error } = await supabase
    .from('subscriptions')
    .select('*')
    .eq('user_id', userId)
...
export const getUpcomingSubscriptions = async (userId: string, days: number = 30): Promise<Subscription[]> => {
...
export const calculateTotalMonthlyCost = async (userId: string): Promise<number> => {
  const { data, error } = await supabase
    .from('subscriptions')
