...
import type { Account, AccountGroup, AccountType, AccountWithGroup } from "./types";
...
export const fetchAccountsWithGroups = async (userId: string): Promise<AccountWithGroup[]> => {
  const { data, error } = await supabase
    .from("accounts")
    .select(`
      *,
...
export const fetchAccountGroups = async (userId: string): Promise<AccountGroup[]> => {
  const { data, error } = await supabase
    .from('account_groups')
    .select('*')
    .eq('user_id', userId);
...
export const getDefaultAccount = async (userId: string): Promise<Account | null> => {
  const { data, error } = await supabase
    .from('accounts')
    .select('*')
    .eq('user_id', userId)
