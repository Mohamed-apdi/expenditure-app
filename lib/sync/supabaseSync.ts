...
import { getPendingChanges, dequeue, recordRetry } from "./changeQueue";

export async function pushPendingToSupabase(): Promise<{ pushed: number; failed: number }> {
  const pending = await getPendingChanges();
  let pushed = 0;
  let failed = 0;
  for (const change of pending) {
    try {
      const table = change.entityType === "transaction" ? "transactions" : change.entityType === "account" ? "accounts" : change.entityType;
...
export async function pullFromSupabaseIntoLocal(userId: string): Promise<void> {
  ...
  const ts = (v: unknown) => (v != null ? `'${String(v).replace(/'/g, "''")}'` : "NULL");
...
export async function runSync(userId: string): Promise<{ pushed: number; failed: number }> {
