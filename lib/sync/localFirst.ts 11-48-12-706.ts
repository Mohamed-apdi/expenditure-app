...
function rowToTransaction(r: Record<string, unknown>): Transaction {
  return {
    ...
    recurrence_interval:
      r.recurrence_interval != null ? String(r.recurrence_interval) : undefined,
...
export async function fetchTransactionsLocal(
  ...
  return rows.map((r: unknown) =>
    rowToTransaction(r as Record<string, unknown>),
  );
}
...
export async function updateTransactionLocal(
  ...
  const db = getLocalDb();
  if (!db) throw new Error("Local DB not available");
  const result = await db.execute("SELECT * FROM transactions WHERE id = ?", [
    transactionId,
  ]);
  const existing = result.rows?._array?.[0] as
...
export async function updateAccountLocal(
  ...
  const db = getLocalDb();
  if (!db) throw new Error("Local DB not available");
  const result = await db.execute("SELECT * FROM accounts WHERE id = ?", [
    accountId,
  ]);
  const existing = result.rows?._array?.[0] as
    | Record<string, unknown>
...
export async function getTransactionByIdLocal(
  transactionId: string,
