...
import type { Expense, ExpenseWithAccount } from './types';
...
export const fetchExpensesWithAccounts = async (userId: string, filters = {}): Promise<ExpenseWithAccount[]> => {
  let query = supabase
    .from('expenses')
    .select(`
      *,
...
export const getExpensesByDateRange = async (userId: string, startDate: string, endDate: string): Promise<Expense[]> => {
  const { data, error } = await supabase
    .from('expenses')
    .select('*')
    .eq('user_id', userId)
